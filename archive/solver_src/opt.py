...
def parse_args():
    ...
    parser.add_argument('--data_path', type=str, default='../data/opt', help='data set folder, for default format see dataset/cora/cora.edges and cora.node_labels')
    ...
    parser.add_argument('--method_name', type=str, default='FCN4', help='method name')
...
def main(args):
    ...
    model = FCN(2, ns, label_dim=ns.shape[1], 
    ...
    for epoch in range(args.epochs):
        ...
        for x, y in train_loader:
            ...
            preds = model(x)
            # unsupervised loss
            #train_loss = -torch.mean(preds[:,args.target]) + args.w*x.shape[-1]*torch.mean(F.relu(x[:,0]-preds) + F.relu(preds-x[:,1])) #+ criterion(preds, y)
